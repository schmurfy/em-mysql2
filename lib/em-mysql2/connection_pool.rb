  ...
  module ConnectionAdapters
    ...
    class ConnectionPool
      ...
      def initialize(spec)
        ...
        @monitor = FiberedMonitor.new
        @queue = @monitor.new_cond
      ...
      # alias_method :checkout_original, :checkout
      # alias_method :checkin_original, :checkin
      
      # def checkout
      #   checkout_original().tap{|ret| p [:checkout, '%x' % ret.object_id] }
      # end
      
      # def checkin(conn)
      #   raise "wtf"
      #   checkin_original(conn).tap{ p [:checkin, '%x' % conn.object_id] }
      # end
    end
